window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const switchPanel = { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, padding: '12px 14px', border: '1px solid rgba(128,128,128,.35)', borderRadius: 9 }
    const switchBase = { position: 'relative', width: 48, height: 28, flex: '0 0 auto', border: 0, borderRadius: 999, padding: 0, cursor: 'pointer', transition: 'background .2s ease' }
    const switchKnob = { position: 'absolute', top: 3, width: 22, height: 22, borderRadius: '50%', background: '#fff', boxShadow: '0 1px 4px rgba(0,0,0,.28)', transition: 'left .2s ease' }
    ...
    function LanLinkCard(props) {
      ...
      const applyResult = result => {
        if (!result || result.ok !== true) {
          if (result?.error?.code === 'port-unavailable') {
            const fallback = result.error.details?.defaultPort ?? 3080
            setPort(String(fallback))
            window.alert(result.error.message)
          }
          throw new Error(result?.error?.message || 'LAN Link 请求失败')
        }
      ...
      const children = [
        ...
        React.createElement('div', { key: 'desc', style: { opacity: .78, lineHeight: 1.55 } },
          '使用 DSH 0.1.5 原生 Web 服务、浏览器认证、启动令牌和签名 Cookie。授权后的局域网用户可完整使用模型、会话、插件和 DSH 操作。配置修改将在重启 DSH Web 后生效。'),
      ...
      if (state) {
        children.push(React.createElement('div', { key: 'toggle', style: switchPanel },
          React.createElement('div', { style: { display: 'grid', gap: 3 } },
            React.createElement('strong', null, '局域网访问'),
            React.createElement('span', { style: { opacity: .68, fontSize: 12 } },
              state.configuredEnabled ? '已设置为开启；重启 DSH 后允许授权设备访问。' : '已设置为关闭；重启 DSH 后仅允许本机访问。')),
          React.createElement('button', {
            type: 'button', role: 'switch', 'aria-checked': state.configuredEnabled,
            'aria-label': state.configuredEnabled ? '关闭局域网访问' : '开启局域网访问',
            disabled: busy || restarting,
            onClick: () => { void call('configure', { enabled: !state.configuredEnabled }) },
            style: { ...switchBase, background: state.configuredEnabled ? '#3478f6' : 'rgba(128,128,128,.3)', opacity: busy || restarting ? .55 : 1 },
          }, React.createElement('span', { style: { ...switchKnob, left: state.configuredEnabled ? 23 : 3 } }))))
