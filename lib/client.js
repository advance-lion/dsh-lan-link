  ...
  factory: (require) => {
    ...
    function LanLinkCard(props) {
      ...
      const call = async (endpoint, payload = null) => {
        setBusy(true)
        try { applyResult(await props.connection.rpc.call(CHANNEL, endpoint, payload)) }
      ...
      if (state) {
        ...
        if (state.restartRequired) children.push(React.createElement('div', { key: 'restart', style: { color: '#d9892b', fontWeight: 600 } },
          'Restart dsh web to apply the saved LAN setting.'))
        children.push(React.createElement('div', { key: 'active', style: { opacity: .75 } },
          `Active bind: ${state.activeHost}:${state.activePort} · DSH ${state.dshVersion} · Cookie ${state.cookieMaxAgeDays} days`))
